            ...
            return (
              ...
              >
                ...
                <div className="ml-16 md:ml-0 md:grid md:grid-cols-2 md:gap-8">
                  {/* Date - left side (odd items) / right side (even items) */}
                  <motion.div
                    variants={{
                      hidden: { opacity: 0, x: -20 },
                      visible: { opacity: 1, x: 0 }
                    }}
                    className={`md:flex md:items-center ${isEven ? 'md:justify-end' : 'md:col-start-2 md:justify-start'}`}
                  >
                    <span className="inline-block px-3 py-1 text-xs font-medium rounded-full bg-green-500/20 border border-green-500/50 text-green-400 mb-2 md:mb-0">
                      {exp.period}
                    </span>
                  </motion.div>

                  {/* Details card - right side (odd items) / left side (even items) */}
                  <motion.div
                    variants={{
                      hidden: { opacity: 0, x: 20 },
                      visible: { opacity: 1, x: 0 }
                    }}
                    className={`bg-zinc-900/50 rounded-2xl border border-zinc-800 p-6 mb-8 md:mb-0 ${isEven ? '' : 'md:col-start-1 md:row-start-1'}`}
                  >
                    <div className="flex flex-col md:items-start gap-2 mb-3">
                      ...
                      <span className="md:hidden inline-block px-3 py-1 text-xs font-medium rounded-full bg-green-500/20 border border-green-500/50 text-green-400">
                        {exp.period}
                      </span>
                    </div>
